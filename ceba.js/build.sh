...
cp -r ./external/tor/src/config ./build/tor_assets

emcc \
  --js-library library_sockfs.js \
  --js-library library_syscall.js \
  -Oz --llvm-lto 1 \
  ...
  -s NO_EXIT_RUNTIME=1 \
  --preload-file ./build/tor_assets/geoip@geoip \
  --preload-file ./build/tor_assets/geoip6@geoip6 \
  -v \
  ./build/tor.bc -o ./build/tor.js

...
function makeSocket(host, port) {
  return wrapSocket(browser.TCPSocket.connect({ host, port }));
}
...
class SocketServer {
  constructor(config) {
    ...
    const startServer = async () => {
      this.server = await browser.TCPSocket.listen({ port: config.port });
...
const randPort = 10000 + Math.floor(Math.random() * 50000);

function handleProxyRequest(requestInfo) {
  // TODO: circuit isolation
  return { type: "socks", host: "127.0.0.1", port: randPort, proxyDNS: true };
}

// TODO: does this catch all tcp connections?
browser.proxy.onRequest.addListener(handleProxyRequest, {urls: ["<all_urls>"]});

const instance = Module({
  CustomSocketServer: SocketServer,
  CustomSocket: TcpSocketWrapper,
  arguments: ['SocksPort', randPort.toString()],
});
